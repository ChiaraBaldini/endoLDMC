...
# from diffusers.models.autoencoders.autoencoder_kl import AutoencoderKL # pretrained model
...
def parse_args():
    ...
    parser.add_argument("--source_model", help="The vae model to fine-tune")
...
class Stage1Wrapper(nn.Module):
    ...
    def __init__(self, model: nn.Module, stack_input_to_rgb:bool =True) -> None:
        super().__init__()
        self.model = model
    ...
    def encode(self, x: torch.Tensor):
        """Expose the encode method of the underlying model."""
        return self.model.encode(x)

    def decode(self, x: torch.Tensor):
        """Expose the decode method of the underlying model."""
...
def main(args):
    ...
    if args.fine_tune:
        logging.info(f"fine_tune={args.fine_tune}: Loading pretrained stage 1 model from {args.source_model} vae") #https://huggingface.co/docs/diffusers/api/pipelines/stable_diffusion/...
