...
def print_gpu_memory_CAPTION():
    if torch.cuda.is_available():
        nvsmi = nvidia_smi.getInstance()
        data = nvsmi.DeviceQuery("memory.used, memory.total, utilization.gpu")["gpu"]
        logging.info("Memory CAPTION")
        for i, data_by_rank in enumerate(data):
            mem_CAPTION = data_by_rank["fb_memory_usage"]
            logging.info(f"gpu:{i} mem(%) {int(mem_CAPTION['used'] * 100.0 / mem_CAPTION['total'])}")
    ...
    for epoch in tqdm(range(start_epoch, n_epochs)):
        ...
        if (epoch + 1) % eval_freq == 0:
            ...
            if val_loss <= best_loss:
                ...
                best_loss = val_loss
                torch.save(checkpoint, vae_path) # Current best model (overwritten)
...
def train_epoch_aekl(
    ...
    for step, x in pbar:
        ...
        with autocast(enabled=True):
            ...
            # latents = model.module.encode(images).latent_dist
            # z_mu = latents.mean
            # z_sigma = latents.std
            # latents_sample = latents.sample()
            # reconstruction = model.module.decode(latents_sample).sample
            output: AutoencoderKLOutput = model.module.encode(images)
            z_mu = output.latent_dist.mean
            z_sigma = output.latent_dist.stddev
            latents_sample = output.latent_dist.sample()
            reconstruction = model.module.decode(latents_sample).sample()
            # Scaling the loss for segmented areas
            if weight_loss_with_mask_by is None:
                l1_loss = F.l1_loss(reconstruction.float(), images.float())
            else:
                mask_batch = x[MASK].to(device)
                weight_tensor = compute_weight_tensor(mask_batch=mask_batch, weight=weight_loss_with_mask_by)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if (epoch + 1) % eval_freq == 0:
            ...
            if val_loss <= best_loss:
                ...
                best_loss = val_loss
                torch.save(checkpoint, model_path) # Current best model (overwritten)
            ...
            # elif (epoch +1) % 20 == 0:
            #     # Routine saving of the model every 20 epochs
...
def train_epoch_ldm(
    ...
    for step, x in pbar:
        ...
        with autocast(enabled=True):
            ...
            prompt_embeds = text_encoder(captions.squeeze(1))
        ...
        if snr_gamma is None:
            ...
            print(f"type(noise_pred): {type(noise_pred)}")
...
def eval_ldm(
    ...
    if sample:
        ...
        log_ldm_sample(
            ...
            device=device,
            scale_factor=scale_factor,
            text_condition=captions[0],
            text_condition_raw=captions[0],
        )
        # Unconditional
        log_ldm_sample(
            model=raw_model,
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if (epoch + 1) % eval_freq == 0:
            ...
            if val_loss <= best_loss:
                ...
                best_loss = val_loss
                torch.save(checkpoint, controlnet_path) # Current best model (overwritten)
            ...
            #     torch.save(checkpoint, str(run_dir / f"controlnet_best_val_loss_{str(val_loss).replace('.','').replace(',','')}_epoch{epoch + 1}.pth")) # model at that epoch (will be persistently stored).
            # if (epoch +1) % 8 == 0:
            #     torch.save(checkpoint, str(run_dir / f"controlnet_best_val_loss_{str(val_loss).replace('.','').replace(',','')}_epoch8.pth")) # model at that epoch (will be persistently stored).
    ...
    for step, x in pbar:
        ...
        with autocast(enabled=True):
            ...
            if scheduler.prediction_type == "v_prediction":
                ...
                target = noise
            random_sampling = random.randint(0, 9999) == 1 # randomly sample every 10000 steps on average
...
def eval_controlnet(
    ...
    for x in tqdm(loader):
        ...
        if sample and counter == 0:
            cond2 = torch.cat((cond, cond, cond), dim=1)
            log_image_pairs(image=cond2, reconstruction=images, model_type='controlnet', writer=writer, step=step,
