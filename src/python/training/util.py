...
def get_datalist(
    ids_path: str,
    default_report_text: str = "Endoscopic image of a larynx with a lesion.",
...
def get_test_dataloader(
    ...
    test_transforms = transforms.Compose(
        [
            ...
            transforms.LoadImaged(keys=[SOURCE, MASK]),
            transforms.EnsureChannelFirstd(keys=[SOURCE, MASK]),
            transforms.Resized(keys=[SOURCE, MASK], spatial_size=(img_height, img_width), mode='bicubic'),# ,mode=InterpolationMode.BICUBIC),
            # # Add if you want to fix flipped image read
            # transforms.Rotate90d(keys=[SOURCE, MASK], k=-1, spatial_axes=(0, 1)),  # Fix flipped image read
            # transforms.Rotate90d(keys=[SOURCE, MASK], k=-1, spatial_axes=(0, 1)),  # Fix flipped image read

            # The image range in diffusers stable diffusion (vae) is in range -1,1
            transforms.ScaleIntensityRanged(keys=[SOURCE, MASK], a_min=0.0, a_max=255.0, b_min=-1.0, b_max=1.0,
                                            clip=True),
            ApplyTokenizerd(keys=[CAPTION]),
            transforms.ToTensord(keys=[SOURCE, MASK, CAPTION]),
        ]
    )

    test_dicts = get_datalist(ids_path=test_ids, use_default_report_text=use_default_report_text)
...
def get_dataloader(
    ...
    val_transforms = transforms.Compose(
        [
            transforms.LoadImaged(keys=[SOURCE]),
            transforms.EnsureChannelFirstd(keys=[SOURCE]),
            transforms.Resized(keys=[SOURCE], spatial_size=(img_height, img_width), mode='bicubic'), #,mode=InterpolationMode.BICUBIC),
        
            transforms.ScaleIntensityRanged(keys=[SOURCE], a_min=0.0, a_max=255.0, b_min=-1.0, b_max=1.0, clip=True),
            ApplyTokenizerd(keys=[CAPTION]),
            transforms.ToTensord(keys=[SOURCE, CAPTION]),
        ]
    )
    if model_type == "autoencoder":
        train_transforms = transforms.Compose(
            [
                transforms.LoadImaged(keys=[SOURCE]),
                transforms.EnsureChannelFirstd(keys=[SOURCE]),
                transforms.Resized(keys=[SOURCE], spatial_size=(img_height, img_width), mode='bicubic'),
                transforms.ScaleIntensityRanged(keys=[SOURCE], a_min=0.0, a_max=255.0, b_min=-1.0, b_max=1.0, clip=True),

                # Data augmentations
                transforms.RandFlipd(keys=[SOURCE], prob=0.1, spatial_axis=0),
                ...
                transforms.RandShiftIntensityd(keys=[SOURCE], offsets=0.05, prob=0.1),
                transforms.RandAdjustContrastd(keys=[SOURCE], gamma=(0.97, 1.03), prob=0.1),
                transforms.ThresholdIntensityd(keys=[SOURCE], threshold=1, above=False, cval=1.0),
                transforms.ThresholdIntensityd(keys=[SOURCE], threshold=0, above=True, cval=0.0),
                # ApplyTokenizerd(keys=[CAPTION]),
                transforms.ToTensord(keys=[SOURCE]),
            ]
        )
    if model_type == "diffusion":
        train_transforms = transforms.Compose(
            [
                ...
                transforms.EnsureChannelFirstd(keys=[SOURCE]),
                transforms.Resized(keys=[SOURCE], spatial_size=(img_height, img_width), mode='bicubic'),
                transforms.ScaleIntensityRanged(keys=[SOURCE], a_min=0.0, a_max=255.0, b_min=-1.0, b_max=1.0,
                                                clip=True),
                ...
                transforms.RandAdjustContrastd(keys=[SOURCE], gamma=(0.97, 1.03), prob=0.1),
                transforms.ThresholdIntensityd(keys=[SOURCE], threshold=1, above=False, cval=1.0),
                transforms.ThresholdIntensityd(keys=[SOURCE], threshold=-1, above=True, cval=-1.0),
                ApplyTokenizerd(keys=[CAPTION]),
    ...
    if model_type == "controlnet":
        val_transforms = transforms.Compose(
            [
                transforms.LoadImaged(keys=[SOURCE, MASK]),
                transforms.EnsureChannelFirstd(keys=[SOURCE, MASK]),
                transforms.Resized(keys=[SOURCE, MASK], spatial_size=(img_height, img_width), mode='bicubic'),
                transforms.ScaleIntensityRanged(keys=[SOURCE, MASK], a_min=0.0, a_max=255.0, b_min=-1.0, b_max=1.0, clip=True),
                ApplyTokenizerd(keys=[CAPTION]),
                transforms.ToTensord(keys=[SOURCE, MASK, CAPTION]),
            ]
        )
        train_transforms = transforms.Compose(
            [
                ...
                # Data augmentations
                transforms.RandFlipd(keys=[SOURCE, MASK], prob=0.1, spatial_axis=0),
                transforms.RandAffined(
                    keys=[SOURCE, MASK],
                    translate_range=(2, 2),
                    scale_range=(0.01, 0.01),
                ...
                transforms.RandShiftIntensityd(keys=[SOURCE, MASK], offsets=0.05, prob=0.1),
                transforms.RandAdjustContrastd(keys=[SOURCE, MASK], gamma=(0.97, 1.03), prob=0.1),
                transforms.ThresholdIntensityd(keys=[SOURCE, MASK], threshold=1, above=False, cval=1.0),
                transforms.ThresholdIntensityd(keys=[SOURCE, MASK], threshold=-1, above=True, cval=-1.0),
                ApplyTokenizerd(keys=[CAPTION]),
                transforms.RandLambdad(
                    keys=[CAPTION],
                    prob=0.10,
...
def get_figure(
    ...
    if img.shape[0] == 1:
        ...
        print(img_npy_0.shape)
        print(recons_npy_0.shape)
        ...
        if recons is not None:
            try:
                ...
                recons_npy_1 = np.clip(a=recons[number_in_batch+1].cpu().numpy(), a_min=-1 if change_figure_min else 0, a_max=1) 
                img_npy_1 = np.clip(a=img[number_in_batch+1].cpu().numpy(), a_min=0, a_max=1) 
                img_npy_0 = np.clip(a=img[number_in_batch].cpu().numpy(), a_min=0, a_max=1)  
        else:
            img_npy_0 = np.clip(a=img[number_in_batch].cpu().numpy(), a_min=-1, a_max=1) 
        ...
        #recons_npy_1 = (recons_npy_1 + 1) * 127.5
...
def log_ldm_sample(
    ...
    # It is assumed that the noise distribution in the LDM training is a uniform distribution on the interval [0, 1) -> mean=0.5, std=1/12
...
def postprocess_and_save_image_triplet(x_hat, target, cond, dpi=100, writer='', step='', conditional_txt ='', dataset_name='', text_condition_raw='', epoch='', enforced_name=None):  #acquisition_time=None,
    ...
    try:
        ...
        # Target is a 3-channel image that still needs to be converted to grayscale
        try:
        ...
        # Image cond_0 is already grayscale with only a single channel (no need to convert to grayscale)
        ...
        if enforced_name is not None:
            ...
            add_custom_figure(fig=fig, writer=writer, step=step, title=f"{conditional_txt}INTERVAL_MAPPED_FROMm11_TO01_GRAYSCALE_SAMPLE",
                          ...
                          text_condition_raw=f"{text_condition_raw}",                      
...
def compute_weight_tensor(segmentation_mask_batch, weight: int=100):
    ...
    # Add +1 to the segmentation mask where the segmentation mask has a value of 0 (background is not ignored (!=0) in the loss function)
...
def weighted_loss(input_tensor, target_tensor, weight_tensor, loss_function='MSE'):
    ...
    out = out * weight_tensor # weight tensor needs to (batch_size, 1 OR 3, height, width)
    loss = out.sum(0)
    return loss
...
def load_controlnet(config, args, diffusion, device):
    if args.init_from_unet:
        ...
        if diffusion is not None and diffusion.config is not None and diffusion.config.num_class_embeds is not None:
            ...
            diffusion.config.num_class_embeds = None 
        controlnet = ControlNetModel.from_unet(unet=diffusion,
                                               #conditioning_embedding_out_channels = (64, 128, 128, 256) 
                                               conditioning_channels=1, # we need to define if we use rgb here or grayscale
...
def save_controlnet_inference_sample(run_dir, filename, sample, cond, images, dpi=100, save_matplotlib=False, save_comparison=True):
    ...
    try:
        ...
        print(sample_postprocessed.shape)
        im = Image.fromarray(np.transpose(sample_postprocessed[0,:,:,:], (1, 2, 0)))
    ...
    if save_comparison:
        ...
        mask_tosave_path=Path(run_dir / "Input_masks" / filename)
        mask_tosave_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            cond_postprocessed = interval_mapping(torch.cat((cond, cond, cond), dim=1).cpu().numpy(), from_min=-1., from_max=1., to_min=0., to_max=1.)
            ...
            # sample_postprocessed = np.expand_dims(np.transpose(sample_postprocessed[0, :, :, :], (1, 2, 0)), axis=0)
            # cond_postprocessed = np.expand_dims(np.transpose(cond_postprocessed[0, :, :, :], (1, 2, 0)), axis=0)
            # images_postprocessed = np.expand_dims(np.transpose(images_postprocessed[0, :, :, :], (1, 2, 0)), axis=0)
            sample_postprocessed = sample_postprocessed[0, :, :, :]  # 3,224,244
            cond_postprocessed = cond_postprocessed[0, :, :, :]
            cond_tosave = Image.fromarray(np.transpose(cond_postprocessed, (1, 2, 0)))
            cond_tosave.save(mask_tosave_path)
            cond_tosave.close()
            images_postprocessed = images_postprocessed[0, :, :, :]
            ...
            im = Image.fromarray(np.transpose(sample_triplet, (1, 2, 0)))
            im.save(output_file_path)
            im.close()
            if Path(output_file_path).exists():
